...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const campaigns = db.prepare(`SELECT c.id, c.name, v.name as vertical FROM campaigns c JOIN verticals v ON v.id = c.vertical_id JOIN customer_campaigns cc ON cc.campaign_id = c.id WHERE cc.customer_id = ? ORDER BY v.name, c.name`).all(id);
  ...
  const comms = db.prepare(`SELECT * FROM communications WHERE customer_id = ? ORDER BY created_at DESC LIMIT 200`).all(id);
