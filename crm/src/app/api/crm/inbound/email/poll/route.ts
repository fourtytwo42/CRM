...
import { runImapPollOnce, getImapPollerStatus, resetImapPollCountdown, ensureImapPollerRunning } from '../poller';
...
export async function POST(req: NextRequest) {
  ...
  // Ensure background poller is initialized so status is accurate
  try { ensureImapPollerRunning(); } catch {}
  const n = await runImapPollOnce();
  // Reset countdown after manual check
  try { resetImapPollCountdown(); } catch {}
  ...
  return jsonOk({ processed: n, head: rows, poller: status });
}
