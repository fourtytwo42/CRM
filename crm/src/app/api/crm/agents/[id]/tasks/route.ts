...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const body = await req.json().catch(() => null) as { title?: string; description?: string; priority?: 'low'|'normal'|'high'|'urgent'; due_date?: string|null };
  const title = String(body?.title || '').trim();
  if (!title) return jsonError('VALIDATION', { status: 400, message: 'Title required' });
  const db = getDb();
  const now = new Date().toISOString();
  db.prepare('INSERT INTO tasks (title, description, status, priority, due_date, created_at, created_by_user_id, assigned_to_user_id) VALUES (?, ?, "open", ?, ?, ?, ?, ?)')
    .run(title, body?.description || null, body?.priority || 'normal', body?.due_date || null, now, me.id, Number(params.id));
  return jsonOk();
}
