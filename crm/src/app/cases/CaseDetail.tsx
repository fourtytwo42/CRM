...
export default function CaseDetail({ caseId, embedded = false }: { caseId: number; embedded?: boolean }) {
    ...
    <div className="py-4">
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {!embedded && (
            <a className="underline" href={`/cases/${caseId}`}>Open in page</a>
          )}
        </div>
      </div>
      ...
      <div className="mb-3 flex items-center gap-2 text-sm">
        {(['Activity','Details','Related'] as const).map(t => (
          <button
            key={t}
            className={`px-3 py-1.5 rounded-lg border ${viewTab===t ? 'bg-black text-white dark:bg-white dark:text-black' : 'bg-transparent text-black dark:text-white'} border-black/10 dark:border-white/10`}
        ...
        <div className="xl:col-span-2 space-y-6">
                ...
                <CardBody>
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="grid grid-cols-1 gap-2 mt-2">
                      <textarea id={`new-note-${caseId}`} placeholder="Add note" className="rounded-lg border px-3 py-2 bg-white dark:bg-gray-900 text-black dark:text-white border-black/10 dark:border-white/10 min-h-[120px]" />
                      <div className="flex items-center justify-end">
                        <Button onClick={async ()=>{
                          const el = document.getElementById(`new-note-${caseId}`) as HTMLTextAreaElement | null; if (!el || !el.value.trim()) return;
                          const token = await getAccessToken(); if (!token) return;
          ...
          {viewTab === 'Details' && (
            <>
              ...
              {/* Version history hidden (still tracked in DB) */}
            </>
          )}
          ...
          {viewTab === 'Related' && (
            <RelatedCasesList
              ...
              fetchCaseNotes={async (cid: number) => {
                try {
                  const token = await getAccessToken(); if (!token) return [] as any[];
                  const res = await fetch(`/api/crm/cases/${cid}`, { headers: { authorization: `Bearer ${token}` } });
                  const j = await res.json().catch(()=>null);
